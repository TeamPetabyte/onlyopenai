...
# ============================================================
# ตั้งเวลารันอัตโนมัติ (สั่งครั้งเดียว ใน PowerShell แบบ Run as Administrator):
#
...
#
# กู้คืน (restore):
#   pg_restore -h localhost -U <DB_USER> -d OpenAI_DB -c "C:\petabyte\backups\OpenAI_DB_<date>.dump"
#
# หมายเหตุ encoding: ข้อความในโค้ดเป็น ASCII ล้วนโดยตั้งใจ — Windows PowerShell 5.1
# อ่านไฟล์ผิด encoding แล้วสตริงไทย/อีโมจิจะทำ parser พัง (เจอมาแล้ว)
...
# .Trim() สำคัญมาก — ไฟล์ .env บน Windows มักเป็น CRLF ถ้าไม่ trim
# ค่า password จะมี \r ติดท้ายแล้วต่อ DB ไม่ได้แบบเงียบๆ
