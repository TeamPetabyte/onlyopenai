...
## 💾 ภาคผนวก B — Backup ฐานข้อมูลอัตโนมัติ (v1.5.1+)

ข้อมูลใน DB (โดยเฉพาะ golden dataset จากการเทรน AI) คือของที่แพงที่สุดในระบบ — ต้องมี backup รายวัน

**ติดตั้งครั้งเดียว** (PowerShell แบบ Run as Administrator บนเครื่อง DB):

```powershell
# 1) ทดสอบรันมือก่อน 1 ครั้ง — ต้องเห็น "✓ backup สำเร็จ"
powershell -ExecutionPolicy Bypass -File C:\petabyte\onlyopenai-master\windows\backup-db.ps1

# 2) ตั้งเวลารันอัตโนมัติทุกวัน 02:00
...
- ไฟล์ backup อยู่ที่ `C:\petabyte\backups\OpenAI_DB_<วันที่>.dump` (เก็บย้อนหลัง 30 วัน, log ใน `backup.log`)
