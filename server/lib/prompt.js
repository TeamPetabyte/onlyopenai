...
- FIX DIRECTLY — provable from the file, and it does not change what the program produces: performance (SELECT inside a loop, SELECT *, missing WHERE, repeated reads), obsolete syntax (TABLES, LIKE, MOVE...TO, header lines), naming conventions, the org's coding standards, a local variable that nothing else in the file references, and a commented-out block of two or more disabled statements in a row with no note saying why it is kept (delete it).
- REPORT AND ASK, never apply — anything that changes the program's behaviour or its output (adding, removing or reordering a condition; applying a variable or SELECT-OPTION the original left unused; changing a derivation, key, filter or sort), and anything this file alone cannot settle (an unused FORM or METHOD that another program may still call — never delete or comment it out; a commented-out block that carries a note saying why it is kept; an unclear relationship between fields).
...
- Precedence when instructions disagree: what the user asked for in THIS message wins; then an exception the answering skill states explicitly for its own subject; then these rules. Knowledge blocks from supporting skills add findings; they never change a decision the answering skill or these rules made. The Accuracy rules below are absolute — no skill overrides them.
