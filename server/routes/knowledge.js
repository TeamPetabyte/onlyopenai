...
// 100MB: large SAP manuals (BC430 PDF ~37MB) must fit; OpenAI's per-file cap is 512MB
const upload = multer({ storage: multer.memoryStorage(), limits: { fileSize: 100 * 1024 * 1024 } });
// same extension set as KB_FILE_RE at boot
...
router.post('/api/knowledge/upload', requireAdmin, expensiveRateLimiter, upload.single('file'), async (req, res) => {
    ...
    // only what sync-knowledge.js can ingest
